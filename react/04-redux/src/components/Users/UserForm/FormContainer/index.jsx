...
function FormContainer({ initialValues, onSubmit, mode, roles, required }) {
  ...
  return (
    <Formik initialValues={initialValues} onSubmit={onSubmit}>
      <Form>
        <div className='form'>
          <Input name={USERNAME} label='Username' disabled={mode === MODE.VIEW || mode === MODE.DELETE}
                 required={required}
                 validate={value => {
                   if (value.trim() === '') {
          ...
          />
          <Input name={AGE} label='Age' disabled={mode === MODE.VIEW || mode === MODE.DELETE} type='number'
                 required={required}
                 validate={value => {
